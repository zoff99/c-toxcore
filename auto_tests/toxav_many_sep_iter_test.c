...
static void *ac_iterate_thread(void *data)
{
    ToxAV *toxav = (ToxAV *)data;
    ...
    printf("Closing ac_iterate thread\n");
...
static void *call_thread(void *pd)
{
    ...
    toxav_audio_iterate_seperation(AliceAV, true);
    toxav_audio_iterate_seperation(BobAV, true);

    pthread_t tids[2];
    (void) pthread_create(&tids[0], nullptr, ac_iterate_thread, (void *)AliceAV);
    (void) pthread_create(&tids[1], nullptr, ac_iterate_thread, (void *)BobAV);
