...
int main(void)
{
    ...
    char ip_str1[IP_STR_MAX_STR_LEN + 1];
    CLEAR(ip_str1);
    tox_friend_get_connection_ip(tox1, 0, (uint8_t *)ip_str1);
    dbg(9, "[%d]:friend2 IP: %s\n", 0, ip_str1);

    char ip_str2[IP_STR_MAX_STR_LEN + 1];
    CLEAR(ip_str2);
    tox_friend_get_connection_ip(tox2, 0, (uint8_t *)ip_str2);
    dbg(9, "[%d]:friend1 IP: %s\n", 1, ip_str2);

    const char *localhost_ip_str = "127.0.0.1 "; // space at the end of the string is intended!

    if (strncmp(localhost_ip_str, ip_str1, strlen(localhost_ip_str)) != 0) {
        dbg(9, "ERROR: ip_str1: LAN 127.0.0.1 is not used!\n");
        tox_kill(tox1);
        tox_kill(tox2);        
        fclose(logfile);
        return -2;
    ...
    if (strncmp(localhost_ip_str, ip_str2, strlen(localhost_ip_str)) != 0) {
        dbg(9, "ERROR: ip_str2: LAN 127.0.0.1 is not used!\n");
        tox_kill(tox1);
        tox_kill(tox2);        
        fclose(logfile);
        return -3;
