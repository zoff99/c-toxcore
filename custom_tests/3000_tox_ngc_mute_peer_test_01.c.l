...
int f_modevent[3] = { 0, 0, 0};
int f_exitevent[3] = { 0, 0, 0};
...
static void group_moderation_cb(Tox *tox, uint32_t group_number, uint32_t source_peer_id, uint32_t target_peer_id,
                                     Tox_Group_Mod_Event mod_type, void *userdata)
{
    tox;
    uint8_t* unum = (uint8_t *)userdata;
    uint8_t num = *unum;
    ...
    {
        dbg(9, "[%d]:group moderation speer:%d tpeer:%d modtype:TOX_GROUP_MOD_EVENT_KICK\n", num, source_peer_id, target_peer_id);    
    }
    else if (mod_type == TOX_GROUP_MOD_EVENT_OBSERVER)
    {
        dbg(9, "[%d]:group moderation speer:%d tpeer:%d modtype:TOX_GROUP_MOD_EVENT_OBSERVER\n", num, source_peer_id, target_peer_id);    
    }
    else if (mod_type == TOX_GROUP_MOD_EVENT_USER)
    {
        dbg(9, "[%d]:group moderation speer:%d tpeer:%d modtype:TOX_GROUP_MOD_EVENT_USER\n", num, source_peer_id, target_peer_id);    
    }
    else if (mod_type == TOX_GROUP_MOD_EVENT_MODERATOR)
    {
        dbg(9, "[%d]:group moderation speer:%d tpeer:%d modtype:TOX_GROUP_MOD_EVENT_MODERATOR\n", num, source_peer_id, target_peer_id);    
    }
    else
    {
        dbg(9, "[%d]:group moderation speer:%d tpeer:%d modtype:%d\n", num, source_peer_id, target_peer_id, mod_type);    
    }
                                      ...
                                      void *userdata)
{
    ...
    dbg(9, "[%d]:group peer status change peer:%d status:%d\n", num, peer_id, status);
}
...
static void set_cb(Tox *tox1, Tox *tox2)
{
    ...
    tox_callback_group_moderation(tox1, group_moderation_cb);
    tox_callback_group_peer_status(tox1, group_peer_status_cb);
    ...
    tox_callback_group_moderation(tox2, group_moderation_cb);
    tox_callback_group_peer_status(tox2, group_peer_status_cb);
    // ---------- CALLBACKS ----------
...
int main(void)
{
    ...
    // ----------- create public NGC group -------------------
    Tox_Err_Group_New err_g1;
    tox_group_new(tox1, TOX_GROUP_PRIVACY_STATE_PUBLIC, (const uint8_t *)"g1", 2, (const uint8_t *)"peer1a", 4, &err_g1);
    dbg(9, "[%d]:created 1st group res=%d\n", 1, err_g1);

    f_join_self[1] = 2; // the group creator does NOT get a self join callback on the first go

    // tox1 invites tox2 to the new public group
    ...
    Tox_Err_Group_State_Queries err_vs;
    Tox_Group_Voice_State vs = tox_group_get_voice_state(tox1, 0, &err_vs);
    dbg(9, "[%d]:group voice state:%d res=%d\n", 1, vs, err_vs);
    vs = tox_group_get_voice_state(tox2, 0, &err_vs);
    dbg(9, "[%d]:group voice state:%d res=%d\n", 2, vs, err_vs);
    // ----------- create public NGC group -------------------


    // ----------- mute tox2 in NGC group -------------------
    Tox_Err_Group_Self_Query err_selfq;
    Tox_Group_Role grole2 = tox_group_self_get_role(tox2, 0, &err_selfq);
    if (grole2 == TOX_GROUP_ROLE_USER)
    {
        dbg(9, "[%d]:friend role in 1st group role=TOX_GROUP_ROLE_USER res=%d\n", 2, err_selfq);
    }
    else if (grole2 == TOX_GROUP_ROLE_OBSERVER)
    {
        dbg(9, "[%d]:friend role in 1st group role=TOX_GROUP_ROLE_OBSERVER res=%d\n", 2, err_selfq);
    }
    else
    {
        dbg(9, "[%d]:friend role in 1st group role=%d res=%d\n", 2, grole2, err_selfq);
    }

    Tox_Err_Group_Mod_Set_Role err_mod;
    tox_group_mod_set_role(tox1, 0, 1, TOX_GROUP_ROLE_OBSERVER, &err_mod);
    dbg(9, "[%d]:muted friend in 1st group res=%d\n", 1, err_mod);
    ...
    grole2 = tox_group_self_get_role(tox2, 0, &err_selfq);
    if (grole2 == TOX_GROUP_ROLE_USER)
    {
        dbg(9, "[%d]:friend role in 1st group after mute role=TOX_GROUP_ROLE_USER res=%d\n", 2, err_selfq);
    }
    else if (grole2 == TOX_GROUP_ROLE_OBSERVER)
    {
        dbg(9, "[%d]:friend role in 1st group after mute role=TOX_GROUP_ROLE_OBSERVER res=%d\n", 2, err_selfq);
    }
    else
    {
        dbg(9, "[%d]:friend role in 1st group after mute role=%d res=%d\n", 2, grole2, err_selfq);
    }
    // ----------- mute tox2 in NGC group -------------------
    ...
    // f_join_self[1] = 0;
    // f_join_self[2] = 0;
    Tox_Err_Group_Disconnect err_gdisc;
    bool g_discon = tox_group_disconnect(tox2, 0, &err_gdisc);
    dbg(9, "[%d]:tox2 leaves group ... res=%d\n", 2, err_gdisc);
    tox_iterate(tox1, (void *)&num1);
    tox_iterate(tox2, (void *)&num2);
    usleep(tox_iteration_interval(tox1) * 1000);
    tox_iterate(tox1, (void *)&num1);
    tox_iterate(tox2, (void *)&num2);
    usleep(tox_iteration_interval(tox1) * 1000);
    Tox_Err_Group_Reconnect err_gconn;
    bool g_conn = tox_group_reconnect(tox2, 0, &err_gconn);
    dbg(9, "[%d]:tries to join again res=%d\n", 2, err_gconn);
    // ------------- tox2 leaves group ----------------------
    ...
    while (1 == 1)
    {
        ...
        if ((f_online[1] > 1) && (f_online[2] > 0) && (f_join_other[1] > 0))
        {
            break;
        }
    }
    ...
    if (grole2 == TOX_GROUP_ROLE_USER)
    {
        dbg(9, "[%d]:friend role in 1st group after offline role=TOX_GROUP_ROLE_USER res=%d\n", 2, err_selfq);
    }
    else if (grole2 == TOX_GROUP_ROLE_OBSERVER)
    {
        dbg(9, "[%d]:friend role in 1st group after offline role=TOX_GROUP_ROLE_OBSERVER res=%d\n", 2, err_selfq);
    }
    else
    {
        dbg(9, "[%d]:friend role in 1st group after offline role=%d res=%d\n", 2, grole2, err_selfq);
    }
