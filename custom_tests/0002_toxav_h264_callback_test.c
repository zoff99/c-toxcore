        ...
        const uint8_t *buf, const uint32_t buf_size, void *user_data) 
{
    ...
    uint8_t* unum = (uint8_t *)user_data;
    uint8_t num = *unum;
//    dbg(9, "[%d]:t_toxav_receive_video_frame_h264_cb\n", num);
    v_frames_rvcd++;
}
...
static void set_cb(Tox *tox1, Tox *tox2, ToxAV *tox1av, ToxAV *tox2av)
{
    ...
    toxav_callback_video_receive_frame_h264(tox1av, t_toxav_receive_video_frame_h264_cb, (void *)&s_num1);
    ...
    toxav_callback_video_receive_frame_h264(tox2av, t_toxav_receive_video_frame_h264_cb, (void *)&s_num2);
...
int main(void)
{
    ...
    for (long looper=0;looper<(5*20);looper++) {
        tox_iterate(tox1, (void *)&num1);
        usleep(5 * 1000);
        tox_iterate(tox2, (void *)&num2);
        usleep(25 * 1000);
    ...
    TOXAV_ERR_CALL_CONTROL call_ctrl_error;
    bool ret = toxav_call_control(toxav1, 0, TOXAV_CALL_CONTROL_CANCEL, &call_ctrl_error);
    dbg(9, "[%d]:toxav_call_control:err=%d err=%d\n", (int)ret, (int)call_ctrl_error);
