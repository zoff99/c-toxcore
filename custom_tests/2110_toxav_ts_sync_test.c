...
#define DELAY_VIDEO_FRAMES_BY_MS 0
...
void putUint64IntoYLayer(uint8_t* y, int width, int height, uint64_t number) {
    for (int i = 0; i < 64; i++) {
        // Get the current bit value
        uint8_t bit = (number >> i) & 1;

        // Set the corresponding pixels in the Y layer
        for (int j = 0; j < 10; j++) {
            for (int k = 0; k < 10; k++) {
                y[(j * width) + (i * 10 + k)] = bit ? 255 : 0; // Set Y value to 255 for white, 0 for black
            }
        }
    }
}

uint64_t readUint64FromYUVFrame(const uint8_t* y) {
    uint64_t readNumber = 0;

    for (int i = 0; i < 64; i++) {
        // Get the Y value of the first pixel in the corresponding bit position
        uint8_t yValue = y[i * 10];

        // Set the bit in the readNumber based on the Y value
        readNumber |= ((uint64_t)yValue >> 7) << i;
    }
    return readNumber;
}
        ...
        void *user_data, uint64_t pts)
{
    ...
    int delta_delta = abs(delta_audio_to_video_ts - (int)DELAY_VIDEO_FRAMES_BY_MS);
    //if (delta_delta > 10)
    {
        dbg(9, "[%d]:XXXXX :%d\n",
            num, delta_delta);
        dbg(9, "[%d]:t_toxav_receive_video_frame_pts_cb %lu %lu (%d) delta_audio_to_video_ts:%d\n",
            num, (unsigned long)cur_ts, (unsigned long)pts, (int)(cur_ts - pts), delta_audio_to_video_ts);
...
void send_av(ToxAV *av)
{
    ...
    char pts_as_txt[300];
    uint64_t t1 = current_time_monotonic_default2();
    uint64_t tt = (uint64_t)((int)t1 - age_v);
    snprintf(pts_as_txt, 299, "%lu", tt);
#if 0
    text_on_yuf_frame_xy_ptr(0, ((h / 2) - (font_height / 2) - 2), pts_as_txt, color_white, y, w, h);
#endif

    // dbg(9, "[%d]: TTin=%lu\n", 1, tt);
    putUint64IntoYLayer(y, w, h, tt);

    uint64_t t2 = current_time_monotonic_default2();
    // dbg(9, "[%d]: dt=%d\n", 1, ((int)t2-(int)t1));

    bool v_sent = toxav_video_send_frame_age(av, 0, w, h, y, u, v, &err, age_v-((int)t2-(int)t1));
    if (v_sent)
    {
        v_frames_sent++;
    }
...
int main(void)
{
    ...
    TOXAV_ERR_OPTION_SET er;
    for (int j=0;j<1000;j++) {
        tox_iterate(tox1, (void *)&num1);
        tox_iterate(tox2, (void *)&num2);
        toxav_option_set(toxav2, 0, TOXAV_DECODER_VIDEO_ADD_DELAY_MS, DELAY_VIDEO_FRAMES_BY_MS, &er);
        if (er == TOXAV_ERR_OPTION_SET_OK)
        {
            break;
        }
        usleep(5 * 1000);
    ...
    dbg(9, "[%d]:toxav_option_set res=%d\n", 1, er);    
    if (er != TOXAV_ERR_OPTION_SET_OK)
    {
        exit(1);
    }
