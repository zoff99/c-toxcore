...
const uint64_t totalf_size = 10 * 1024 * 1024;
                               ...
                               void *userdata)
{
    ...
    if (control == 2)
    {
        ...
        dbg(9, "[%d]:ERR:file_recv_control. stop receiving FT\n", num);
        ft_cancel[num] = 1;
    }
}
...
int main(void)
{
    ...
    tox_kill(tox2);
    tox2 = NULL;
    dbg(9, "[%d]:sleeping for 60 seconds ...\n", 0);
    usleep(60*1000*1000);
    ...
    dbg(9, "[%d]:friends online AGAIN\n", 0);

    dbg(9, "[%d]:!!Notice!! it will take about 2 minutes to timeout the FT on the receiver side ...\n", 0);
    // HINT: it takes STALE_CYCLES_ON_FTV2_RECEIVER tox_iteration intervals for the receiver to detect stale FTv2s
    // STALE_CYCLES_ON_FTV2_RECEIVER = 2000
    // 2000 * 50ms =~ 100 secs =~ 1.6 minutes

    uint64_t ts_start = current_time_monotonic_default2();
    ...
    while (1 == 1) {
        ...
        if (ft_cancel[1] == 1)
        {
            ...
            //       the receiver got the signal to cancel the non existing FTv2.
            uint64_t ts_end = current_time_monotonic_default2();
            dbg(9, "[%d]:it took %d seconds to detect the stale filetransfer.\n", 0, (int)((ts_end - ts_start) / 1000));
            break;
        }
    }
