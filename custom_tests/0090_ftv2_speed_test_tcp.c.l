...
const uint64_t totalf_size = 50L * 1024L * 1024L;
...
#define SPEED_STEPS1 200
#define SPEED_STEPS (SPEED_STEPS1 * 1024)
    if (bytesPerSecond >= SPEED_STEPS) {
        int result = (int)(bytesPerSecond / SPEED_STEPS);
        if (result <= 100)
        {
            sprintf(outstr, "%d kB/s: %.*s", SPEED_STEPS1, result, "#########################################################################################################################################################################################################################################################");
        }
        else
        {
            sprintf(outstr, "%d kB/s: ===>>>>>>>>>>>>>", SPEED_STEPS1);
        }
    }
    else {
        sprintf(outstr, "%d kB/s: ", SPEED_STEPS1);
    ...
    else
    {
        ...
        if ((print_counter % 770) == 0) // (770 * 1363) =~ 1MiB
        {
            ...
            else
            {
                ...
                // dbg(9, "[%d]: -> %d %ld %ld\n", num, time_delta_ms, byte_delta, transfer_speed);
                char outstring1[2000];
                memset(outstring1, 0, 2000);
                printBytesPerSecond(transfer_speed, outstring1);
                // dbg(9, "[%d]: -> file_recv_chunk speed=%s\n", num, outstring1);
                char outstring[2000];
                memset(outstring, 0, 2000);
                printSpeedBar(transfer_speed, outstring);
                dbg(9, "[%d]: -> file_recv_chunk speed=%s %s\n", num, outstring1, outstring);
                time_ts = end_ts;
...
int main(void)
{
    ...
    while (1 == 1) {
        tox_iterate(tox2, (void *)&num2);
        usleep(tox_iteration_interval(tox2)*500);
        tox_iterate(tox1, (void *)&num1);
        usleep(tox_iteration_interval(tox1)*500);
        if (ft_fin == 1)
        {
            break;
        }
