...
const uint64_t totalf_size = 5L * 1024L * 1024L;
uint64_t cur_recv_pos = 0;
uint64_t cur_recv_for_speed_pos = 0;
uint64_t cur_send_pos = 0;
uint32_t print_counter = 0;
uint64_t time_ts = 0;
uint64_t overall_time_ts = 0;
int64_t file_receive_num = -1;
...
static void printBytesPerSecond(long bytesPerSecond, char *outstr) {
    if (bytesPerSecond < 1024) {
        sprintf(outstr, "%ld B/s", bytesPerSecond);
    } else if (bytesPerSecond < 1024 * 1024) {
        sprintf(outstr, "%.2f KB/s", (float)bytesPerSecond / 1024);
    } else if (bytesPerSecond < 1024 * 1024 * 1024) {
        sprintf(outstr, "%.2f MB/s", (float)bytesPerSecond / (1024 * 1024));
    } else {
        sprintf(outstr, "%.2f GB/s", (float)bytesPerSecond / (1024 * 1024 * 1024));
    }
    return outstr;
}

static void printSpeedBar(long bytesPerSecond, char *outstr) {
    if (bytesPerSecond >= 1 * 1024 * 1024) {
        int result = (int)(bytesPerSecond / (1024 * 1024));
        if (result <= 100)
        {
            sprintf(outstr, "MB/s: %.*s", result, "##############################################################################################################################################################################################################################################################################");
        }
        else
        {
            sprintf(outstr, "MB/s: ===>>>>>>>>>>>>>");
                       ...
                       size_t length, void *userdata)
{
    ...
    if (data == NULL)
    {
        ...
        dbg(9, "[%d]: -> file_recv_chunk FINISHED:RECEIVER:==================================\n", num);
        dbg(9, "[%d]:ID:F: %.*s\n", num, TOX_FILE_ID_LENGTH * 2, key_str_chunk);
        dbg(9, "[%d]: -> file_recv_chunk FINISHED:RECEIVER filenum=%d pos=%lu length=%d\n", num, filenumber, position, (int)length);

        uint64_t end_ts = current_time_monotonic_default2();
        long time_delta_ms = (long)(end_ts - overall_time_ts);
        long byte_delta = position;
        long transfer_speed = (long)((float)byte_delta / ((float)time_delta_ms / 1000.0f));
        if (transfer_speed < 0)
        {
            transfer_speed = 0;
        }
        // dbg(9, "[%d]: -> %d %ld %ld\n", num, time_delta_ms, byte_delta, transfer_speed);
        char outstring[200];
        memset(outstring, 0, 200);
        printBytesPerSecond(transfer_speed, outstring);
        dbg(9, "[%d]: -> file_recv_chunk overall speed=%s\n", num, outstring);
        dbg(9, "[%d]: -> file_recv_chunk FINISHED:RECEIVER: ... DONE\n", num);
        dbg(9, "[%d]: -> file_recv_chunk FINISHED:RECEIVER:==================================\n", num);
    }
    else
    {
        if (position == 0)
        {
            overall_time_ts = current_time_monotonic_default2();
            dbg(9, "[%d]: -> file_recv_chunk set overall_time_ts\n", num);
        }
        ...
        if ((print_counter % 770) == 0) // (770 * 1363) =~ 1MiB
        {
            ...
            if (time_ts == 0)
            {
                time_ts = current_time_monotonic_default2();
            }
            else
            {
                uint64_t end_ts = current_time_monotonic_default2();
                int time_delta_ms = (int)(end_ts - time_ts);
                long byte_delta = (int)(position - cur_recv_for_speed_pos);
                long transfer_speed = (long)((float)byte_delta / ((float)time_delta_ms / 1000.0f));
                if (transfer_speed < 0)
                {
                    transfer_speed = 0;
                ...
                // dbg(9, "[%d]: -> %d %ld %ld\n", num, time_delta_ms, byte_delta, transfer_speed);
                char outstring[2000];
                memset(outstring, 0, 2000);
                printBytesPerSecond(transfer_speed, outstring);
                dbg(9, "[%d]: -> file_recv_chunk speed=%s\n", num, outstring);
                memset(outstring, 0, 2000);
                printSpeedBar(transfer_speed, outstring);
                dbg(9, "[%d]: -> file_recv_chunk %s\n", num, outstring);
                time_ts = end_ts;
                cur_recv_for_speed_pos = position;
            }
        }
                                   ...
                                   size_t length, void *userdata)
{
    ...
    uint8_t *f_data = malloc(length + TOX_FILE_ID_LENGTH);
                             ...
                             const uint8_t *filename, size_t filename_length, void *userdata)
{
    ...
    dbg(9, "[%d]:file_receive:%s size=%lu kind=%d filenum=%d\n", num, filename, filesize, kind, file_number);
    ...
    file_receive_num = (int64_t)file_number;
}

static void file_received(Tox *tox, uint32_t friend_number, uint32_t file_number, uint8_t num)
{
    Tox_Err_File_Control error;
    bool res1 = tox_file_control(tox, friend_number, file_number, TOX_FILE_CONTROL_RESUME, &error);
    dbg(9, "[%d]:file_receive:============>>>>>> friendnum=%d filenum=%d res1=%d err=%d\n", num, friend_number, file_number, (int)res1, error);
}
...
int main(void)
{
    ...
    Tox_Err_File_Send error_ft1;
    uint32_t fnum = tox_file_send(tox2, 0, TOX_FILE_KIND_FTV2, totalf_size, NULL, (const uint8_t *)"Gentoo.exe",
                                  sizeof("Gentoo.exe"), &error_ft1);
    dbg(9, "[%d]:file send:started:fnum=%d errnum=%d\n", 2, fnum, error_ft1);
...
/*
    dbg(9, "[%d]:friends online:iterate only the sender:SSS01\n", 0);
    int loops2 = 0;
    while (loops2 < 700) {
        loops2++;
        tox_iterate(tox2, (void *)&num2);
...
*/
    ...
    dbg(9, "[%d]:friends online:wait for incoming file\n", 0);
    while (file_receive_num == -1) {
        tox_iterate(tox2, (void *)&num2);
        usleep(1*1000);
        tox_iterate(tox1, (void *)&num1);
        usleep(9*1000);
...
/*
    dbg(9, "[%d]:friends online:iterate only the receiver:RRR01\n", 0);
    int loops3 = 0;
    while (loops3 < 5) {
        loops3++;
        tox_iterate(tox1, (void *)&num2);
        usleep(100*1000);
    }
*/

    dbg(9, "[%d]:friends online:accept FT while sender is offline\n", 0);
    file_received(tox1, 0, (uint32_t)file_receive_num, 1);

    dbg(9, "[%d]:friends online:iterate only the receiver:RRR02\n", 0);
    int loops2 = 0;
    while (loops2 < 700) {
        loops2++;
        tox_iterate(tox1, (void *)&num2);
    ...
    dbg(9, "[%d]:friends online:iterate both toxes again\n", 0);
    while (1 == 1) {
        tox_iterate(tox2, (void *)&num2);
        usleep(1*1000);
        tox_iterate(tox1, (void *)&num1);
