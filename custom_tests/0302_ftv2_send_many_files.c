...
FILE *logfile = NULL;
#define PARALLEL_FILES 3
uint8_t *key_bin_rec[PARALLEL_FILES];

const uint64_t totalf_size = 400L * 1024L;
...
int main(void)
{
    ...
    while (loop_counter < 620)
    {
        ...
        for (int k=0;k<PARALLEL_FILES;k++)
        {
            ...
            file_counter++;
            snprintf(this_filename, 299, "Gentoo_%d_.exe", file_counter);
        ...
        for (int i=0;i<10;i++) {
            tox_iterate(tox1, (void *)&num1);
            usleep(tox_iteration_interval(tox1)*100);
            tox_iterate(tox2, (void *)&num2);
            usleep(tox_iteration_interval(tox2)*100);
        }
